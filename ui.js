...
document.addEventListener('DOMContentLoaded', () => {
  ...
  window.openOverlay = function(item, id) {
    ...
    // Seller name click → open seller profile
    const overlaySellerEl = document.getElementById('overlaySeller');
    if (overlaySellerEl) {
      overlaySellerEl.onclick = () => {
        if (item.sellerUid) openSellerProfile(item.sellerUid, item.seller);
      };
    }
  ...
  async function openSellerProfile(sellerUid, sellerName) {
    const overlay   = document.getElementById('sellerProfileOverlay');
    const avatarEl  = document.getElementById('sellerProfileAvatar');
    const nameEl    = document.getElementById('sellerProfileName');
    const ratingEl  = document.getElementById('sellerProfileRating');
    const itemsEl   = document.getElementById('sellerProfileItems');
    const reviewsEl = document.getElementById('sellerProfileReviewsList');
    const listCount = document.getElementById('sellerProfileListings');
    const viewCount = document.getElementById('sellerProfileViews');
    const revCount  = document.getElementById('sellerProfileReviews');

    // Reset
    nameEl.textContent    = sellerName || 'Seller';
    ratingEl.textContent  = '';
    avatarEl.style.backgroundImage = '';
    avatarEl.textContent  = (sellerName || '?').split(' ').map(w => w[0]).join('').slice(0,2).toUpperCase();
    itemsEl.innerHTML     = '<p class="reviews-empty">Loading…</p>';
    reviewsEl.innerHTML   = '<p class="reviews-empty">Loading…</p>';
    ...
    try {
      // Fetch seller Firestore profile for photo
      ...
      const itemsSnap = await db.collection('items')
        .where('sellerUid', '==', sellerUid).get();

      listCount.textContent = itemsSnap.size;
      let totalViews = 0;
      itemsSnap.forEach(doc => { totalViews += doc.data().views || 0; });
      viewCount.textContent = totalViews;

      // Render mini item cards
      if (itemsSnap.empty) {
        itemsEl.innerHTML = '<p class="reviews-empty">No listings yet</p>';
      } else {
        itemsEl.innerHTML = '';
        itemsSnap.forEach(doc => {
          const d    = doc.data();
          const img  = d.imageUrls && d.imageUrls[0] ? d.imageUrls[0] : '';
          const card = document.createElement('div');
          card.className = 'seller-mini-card';
          card.innerHTML = `
            <div class="seller-mini-img" ${img ? `style="background-image:url('${img}')"` : ''}>
              ${!img ? '📦' : ''}
            </div>
            <div class="seller-mini-info">
              <p class="seller-mini-title">${d.title}</p>
              <p class="seller-mini-price">₦${Number(d.price).toLocaleString()}</p>
            </div>
          `;
          card.addEventListener('click', () => {
            closeSellerProfile();
            if (window.openOverlay) window.openOverlay(d, doc.id);
          });
          itemsEl.appendChild(card);
        });
      }

      // Fetch all reviews across seller's items
      let allReviews = [], totalRating = 0;
      await Promise.all(itemsSnap.docs.map(async doc => {
        const rSnap = await db.collection('reviews').doc(doc.id)
          .collection('userReviews').get();
        rSnap.forEach(r => {
          allReviews.push({ ...r.data(), itemTitle: doc.data().title });
          totalRating += r.data().rating || 0;
        });
      }));

      revCount.textContent = allReviews.length;

      if (allReviews.length > 0) {
        const avg   = (totalRating / allReviews.length).toFixed(1);
        const stars = '★'.repeat(Math.round(avg)) + '☆'.repeat(5 - Math.round(avg));
        ratingEl.textContent = `${stars} ${avg} (${allReviews.length} reviews)`;

        reviewsEl.innerHTML = '';
        allReviews.forEach(r => {
          const starStr  = '★'.repeat(r.rating) + '☆'.repeat(5 - r.rating);
          const initials = (r.reviewerName || '?').split(' ').map(w => w[0]).join('').slice(0,2).toUpperCase();
          const card = document.createElement('div');
          card.className = 'review-card';
          card.innerHTML = `
            <div class="review-card-header">
              <div class="review-card-avatar">${initials}</div>
              <span class="review-card-name">${r.reviewerName || 'User'}</span>
              <span class="review-card-stars">${starStr}</span>
            </div>
            <p class="review-card-date" style="font-size:10px;color:#9ca3af;margin:2px 0 4px">on: ${r.itemTitle || ''}</p>
            ${r.comment ? `<p class="review-card-text">${r.comment}</p>` : ''}
          `;
          reviewsEl.appendChild(card);
        });
      } else {
        reviewsEl.innerHTML = '<p class="reviews-empty">No reviews yet</p>';
      }
  ...
  const _sellerProfileClose = document.getElementById('sellerProfileClose');
  if (_sellerProfileClose) _sellerProfileClose.addEventListener('click', closeSellerProfile);

  const _sellerProfileOverlay = document.getElementById('sellerProfileOverlay');
  if (_sellerProfileOverlay) _sellerProfileOverlay.addEventListener('click', e => {
    if (e.target === _sellerProfileOverlay) closeSellerProfile();
  });

  // Swipe to close
  const _sellerSheet = document.querySelector('#sellerProfileOverlay .overlay-sheet');
  if (_sellerSheet) {
    let spTouchY = 0;
  ...
  window.addEventListener('beforeinstallprompt', (e) => {
    ...
    // Always show install banner — remove dismissed flag on production
    localStorage.removeItem('unimart_install_dismissed');
    if (installBanner) {
  ...
  async function loadAnnouncements() {
    try {
      ...
      console.log('Announcements fetched:', snap.size, 'docs');

      if (snap.empty) {
        console.log('Announcements collection is empty');
        return;
      }
